...
describe('SOOYA 1-9 feature regressions', () => {
  ...
  it('persists reply targets and withdraws by preserving a visible placeholder', async () => {
    ...
    const second = await harness.app.server.inject({
      ...
      payload: { clientMsgId: 'c_reply', replyTo: firstId, content: [{ type: 'text', text: '这是引用回复' }] }
    });
  ...
  it('moves gallery media to trash, restores favorites, and blocks referenced permanent deletion', async () => {
    ...
    const response = await harness.app.server.inject({ method: 'DELETE', url: `/api/admin/media/${row.id}/permanent` });
  ...
  it('removes invalid push endpoints after 404 and previews TTS without polluting chat history', async () => {
    harness = await createHarness({ tts: 'ok' });
    ...
    const preview = await harness.app.server.inject({ method: 'POST', url: '/api/admin/voice/preview', payload: { text: '温柔地说一句话', emotion: 'gentle' } });
