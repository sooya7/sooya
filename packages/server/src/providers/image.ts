...
/**
 * The single choke point every generated image passes through, whichever
 * provider produced it. Checks what actually came back against
...
export class OpenAIImageProvider implements ImageProvider {
  ...
  async generate(
    ...
    if (refs.length === 1) {
      const ref = refs[0]!;
      return this.edit(prompt, ref.data, { mime: ref.mime, signal: opts.signal, size: opts.size ?? this.cfg.size });
    }
    return withRetry(
      async () => {
  ...
  async edit(
    prompt: string,
    image: Buffer,
    opts: { mime?: string; signal?: AbortSignal; size?: string } = {}
  ): Promise<GeneratedImage> {
    if (!this.configured) throw new ProviderNotConfiguredError('image');
    ...
    try {
      ...
      const json = (await res.json()) as ImageApiResponse;
      // The form now carries `size`, so provenance compares against what was
      // actually sent. Passing `null` here used to be described as "skip the
      // check", but reportProvenance resolves `requestedSize ?? cfg.size`, so it
      // still compared against the configured size while the request never
      // mentioned one — every reference reply logged a mismatch it could not win.
