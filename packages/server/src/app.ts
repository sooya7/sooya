...
import { closeDatabase, openDatabase, reconcileCounters, type Db } from './db/index.js';
...
export interface BuildAppOptions {
  ...
  logger?: Logger;
  /** Skip importing the built-in sticker pack (tests). */
  skipStickerImport?: boolean;
  assetsDir?: string;
  /** Override fetch for provider adapters (tests). */
...
export async function buildApp(opts: BuildAppOptions = {}): Promise<SooyaApp> {
  ...
  const logger =
    ...
    createLogger({
      level: env.LOG_LEVEL,
      logDir: env.NODE_ENV === 'test' ? null : env.logDir,
      pretty: env.NODE_ENV === 'development'
    });

  for (const dir of [env.dataDir, env.dbDir, env.mediaDir, env.backupDir, env.logDir, ...Object.values(env.mediaDirs)]) {
    ensureDirSync(dir);
  }
  ...
  // All repositories talk to the handle, never to a captured connection, so a
  // backup restore can swap the underlying file without stale references.
  ...
  const context = new ContextBuilder(repos.messages, repos.summaries, memory, repos.media, mediaStore);
  ...
  const backups = new BackupService({
    ...
    reopenAfterRestore: () => {
      const reopened = openDatabase({ file: dbFile, backupDir: env.backupDir, onLog: (l, m, e) => logger[l]({ ...e }, m) });
      dbHandle.swap(reopened.db);
    },
    readCounters: () => ({
      messageSeq: repos.messages.maxSeq(),
  ...
  // Reserved agent architecture (inactive in v1).
  ...
  for (const cap of ['chat', 'vision', 'summary', 'embedding', 'image', 'tts', 'stt'] as const) {
    agentCapabilities.register({
      name: cap,
      description: `model capability: ${cap}`,
      available: () => capabilities.has(cap)
    });
  }

  // Import the built-in sticker pack once.
  ...
  // A crash (or a hand-restored database file) can leave the counters below the
  // rows that already exist, which would make the next insert collide with an
  // existing seq. Reconcile before serving any traffic.
  const startupCounters = reconcileCounters(dbHandle);
  logger.debug(startupCounters, 'sequence counters reconciled at startup');

  // Crash recovery: requeue interrupted jobs, mark orphaned sending messages.
  const requeued = repos.jobs.recoverStuck();
  if (requeued > 0) logger.warn({ requeued }, 'requeued interrupted jobs');
  const orphaned = dbHandle
    .prepare("UPDATE messages SET status = 'failed', error = 'interrupted by restart' WHERE status = 'sending'")
    .run().changes;
  ...
  const state = {
    ...
    dbRecovered: opened.recovered,
    dbRecoveredFrom: opened.recoveredFrom,
    version: VERSION
  };
  ...
  // Static web client (built SPA) when present.
  ...
  if (webDir && fs.existsSync(path.join(webDir, 'index.html'))) {
    ...
    server.setNotFoundHandler((req, reply) => {
      ...
      // Only real navigations get the SPA shell. Serving index.html for a
      // missing /assets/*.js would trigger a strict-MIME failure in the
      // browser and hide the actual 404.
...
function scheduleRecurring(app: SooyaApp): void {
  ...
  if (env.BACKUP_INTERVAL_MS > 0) {
    const backup = setInterval(() => {
      try {
        repos.jobs.enqueue('backup.create', { reason: 'scheduled' });
      } catch {
        /* ignore */
      }
  ...
  if (env.BACKUP_ON_START) {
    repos.jobs.enqueue('backup.create', { reason: 'startup' });
  }
}
