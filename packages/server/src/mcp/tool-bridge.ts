...
import type { McpRemoteTool, McpServerConfig, McpToolPolicy } from './types.js';

export function bridgeMcpTools(config: McpServerConfig, connection: McpConnection, remoteTools: McpRemoteTool[]): ToolDescriptor[] {
  return remoteTools.map((remote) => {
    ...
    return {
      ...
      handler: async (input: unknown, context) => wrapMcpResult(await connection.callTool(remote.name, asObject(input), context?.signal))
    };
  });
}
...
function defaultPolicy(serverId: string, remoteName: string): McpToolPolicy {
  if (serverId === 'ombre') {
    if (['breath', 'breath_search', 'breath_advanced', 'pulse', 'letter_read'].includes(remoteName)) {
