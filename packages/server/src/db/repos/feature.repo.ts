...
export class WorldRepo {
  ...
  list(input: { search?: string; kind?: WorldKind; active?: boolean; limit?: number; offset?: number } = {}): WorldEntryRow[] {
    ...
    if (input.search?.trim()) {
      const q = literalContainsPattern(input.search.trim());
      where.push(`(
        subject LIKE ? ESCAPE '\\' OR predicate LIKE ? ESCAPE '\\' OR
        object LIKE ? ESCAPE '\\' OR value_json LIKE ? ESCAPE '\\'
      )`);
      values.push(q,q,q,q);
    }
  ...
  relevant(query: string, limit = 18): WorldEntryRow[] {
    ...
    const clauses = terms.map(() => `(subject LIKE ? ESCAPE '\\' OR predicate LIKE ? ESCAPE '\\' OR object LIKE ? ESCAPE '\\')`).join(' OR ');
    const values: unknown[] = terms.flatMap((term) => {
      const q = literalContainsPattern(term);
      return [q, q, q];
    });
    values.push(limit);
  ...
  update(id: string, patch: Partial<WorldCandidate> & { active?: boolean }): WorldEntryRow | undefined {
    ...
    this.db.prepare('UPDATE world_entries SET kind=?,subject=?,subject_key=?,predicate=?,predicate_key=?,object=?,value_json=?,confidence=?,authority=?,active=?,conflict_of=?,updated_at=? WHERE id=?').run(normalized.kind,normalized.subject,worldIdentityKey(normalized.subject),normalized.predicate,worldIdentityKey(normalized.predicate),normalized.object,JSON.stringify(normalized.value ?? {}),normalized.confidence ?? 0.6,normalized.authority ?? 'model',active,conflictOf,nowIso(),id);
    return this.get(id);
  }
...
function normalize(value:string):string { return worldIdentityKey(value).replace(/[\s，。！？、；：,.!?;:]+/g,''); }
