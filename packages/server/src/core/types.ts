...
/** Part payload accepted when creating a message. */
...
export const SendMessageSchema = z.object({
  ...
  /** Optional explicit user directives, also parsed from the text. */
  directives: z
    .object({
      wantSticker: z.boolean().optional(),
      wantImage: z.boolean().optional(),
      wantVoice: z.boolean().optional(),
      voiceOnly: z.boolean().optional(),
      noSticker: z.boolean().optional(),
      anotherSticker: z.boolean().optional()
    })
    .optional()
});
...
export interface MemoryRecord {
  id: string;
  kind: 'profile' | 'preference' | 'relationship' | 'project' | 'event' | 'summary';
  content: string;
  importance: number;
  confidence: number;
  createdAt: string;
