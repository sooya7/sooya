...
function ImagePart({ part }: { part: MessagePart }) {
  ...
  if (part.status === 'failed') {
    return (
      <div className="bubble bubble-note">
        图片没有发出去{part.error ? `：${part.error}` : ''}
      </div>
    );
  }
  if (part.status === 'pending' || !part.media) {
    return <div className="bubble bubble-note pulsing">图片生成中…</div>;
  }
  ...
  return (
    <a className="image-part" href={mediaUrl(part.media.url)} target="_blank" rel="noreferrer">
      <img
...
function StickerPart({ part }: { part: MessagePart }) {
  ...
  return (
    <img
      className="sticker-part"
      src={mediaUrl(part.media.url)}
      alt={String(part.meta?.stickerName ?? '表情')}
      loading="lazy"
      onError={() => setFailed(true)}
    />
  );
    ...
    <a className="bubble bubble-file" href={mediaUrl(part.media.url)} target="_blank" rel="noreferrer" download>
      <span className="file-icon" aria-hidden="true">
        <svg viewBox="0 0 24 24" width="22" height="22">
          ...
          />
          <path d="M14 2v5h5" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round" />
        </svg>
      </span>
      <span className="file-meta">
        <span className="file-name">{part.media.name ?? '文件'}</span>
        <span className="file-size">{formatBytes(part.media.bytes)}</span>
      </span>
    </a>
  );
}
  ...
  if (message.role === 'system') {
    return (
      <div className="system-row">
        <span>{message.content.map((p) => p.text).filter(Boolean).join(' ')}</span>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="avatar-slot">
        {showAvatar && (
          <img className="avatar" src={mine ? userAvatar : avatar} alt={mine ? '我' : personaName} draggable={false} />
        )}
      </div>
        ...
        <div className="bubbles">
          {visible.map((part) => {
            switch (part.type) {
              case 'text':
                return part.text ? (
                  <div key={part.id} className={`bubble bubble-text ${mine ? 'mine' : 'theirs'}`} data-testid="text-bubble">
                    {part.text}
                  </div>
                ) : null;
              case 'sticker':
                return <StickerPart key={part.id} part={part} />;
              case 'image':
                return <ImagePart key={part.id} part={part} />;
              case 'audio':
                return <AudioBubble key={part.id} part={part} mine={mine} />;
              case 'file':
