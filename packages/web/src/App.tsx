...
function ChatApp() {
  ...
  return <div className="app">
    ...
    {unread > 0 && !stickToBottom && <button type="button" className="unread-pill" onClick={jumpToBottom}>{unread} 条新消息 ↓</button>}
    ...
    <Composer disabled={chat.connection === 'connecting' && !chat.ready} onSend={async (content) => { const result = await chat.send(content, undefined, quote?.id); setQuote(null); return result; }} onNotice={setNotice} />
  </div>;
}
