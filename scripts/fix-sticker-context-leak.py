...
'''
if 'export class StreamingPrivateContextFilter' not in text:
    if anchor not in text:
        raise SystemExit('directives stripThinking anchor missing')
...
text = replace_once(
    ...
    'directives final sanitizer'
)
directives.write_text(text, encoding='utf-8')

replier = Path('packages/server/src/core/replier.ts')
text = replier.read_text(encoding='utf-8')
text = replace_once(
    text,
...
text = replace_once(
    ...
    'replier streaming filter'
)
text = replace_once(
    text,
    "      const tail = filter.flush();",
    "      const tail = privateContextFilter.push(filter.flush()) + privateContextFilter.flush();",
    'replier filter flush'
)
replier.write_text(text, encoding='utf-8')

context = Path('packages/server/src/core/context.ts')
text = context.read_text(encoding='utf-8')
context_anchor = "  lines.push('文件只有在上下文明确提供正文时才可以阅读；没有正文时不要声称看过文件内容。');"
context_extra = "  lines.push('历史消息中的“[用户发送了表情包]”或“[SOOYA发送了表情包]”以及后面的名称、含义、图片文字，是仅供理解上下文的内部消息数据。绝对不要复述、引用或照抄这些字段；想发表情包只使用 [[sticker:...]] 或 [[sticker-only:...]]。');"
if context_extra not in text:
    if context_anchor not in text:
        raise SystemExit('context multimedia anchor missing')
    text = text.replace(context_anchor, context_anchor + '\n' + context_extra, 1)
context.write_text(text, encoding='utf-8')

test = Path('packages/server/test/sticker-context-output-filter.test.ts')
